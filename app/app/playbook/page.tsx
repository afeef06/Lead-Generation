...
const NICHE_NOTES: NicheNote[] = [
  { niche: 'Personal Injury Law Firms',      color: '#5A7A9E', note: 'Highest budgets. One-time $3,000–$8,000+, retainers $1,000–$2,000+/mo. Pitch speed-to-lead AI (they lose cases to faster competitors). Always aim for full AI automation.' },
  { niche: 'Med Spas & Aesthetic Clinics',   color: '#9E5A9E', note: 'Strong budgets. One-time $2,000–$5,000, retainers $800–$1,500/mo. Focus on booking automation + rebooking campaigns. Owners are ROI-driven.' },
  { niche: 'Dental & Orthodontic Practices', color: '#2A8C68', note: 'Reliable buyers. One-time $1,500–$3,500, retainers $400–$900/mo. Lead with appointment reminders + patient reactivation. Front desk pain is the hook.' },
...
const SALES_GOALS: GoalSet[] = [
  {
    period: 'Daily', color: '#2A8C68', bg: '#0E2A1E', border: '#2A8C68',
    items: [
  ...
  {
    period: 'Weekly', color: '#9E7A30', bg: '#1A160A', border: '#9E7A30',
    items: [
  ...
  {
    period: 'Monthly', color: '#5A7A9E', bg: '#0E1A2A', border: '#5A7A9E',
    items: [
          ...
          {process.map(({ num, title, body, tips }) => (
            <div key={num} className="bg-[#0D0D0D] border border-[#1A1A1A] rounded-xl p-4">
              <div className="flex items-start gap-3 mb-2">
                <span className="w-6 h-6 rounded-lg bg-[#1A1A1A] border border-[#2A2A2A] flex items-center justify-center text-[10px] font-bold text-[#5A7A9E] flex-shrink-0 mt-0.5">{num}</span>
              ...
              {tips && tips.length > 0 && (
                <div className="mt-3 pl-9 space-y-1.5">
                  {tips.map((tip, i) => (
                    <p key={i} className="text-xs text-[#5A7A9E] flex items-start gap-1.5">
      ...
      {tab === 'pricing' && (
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: 'Entry Point',      value: '$400',       sub: 'AI Strategy Session', color: '#9E7A30',  Icon: DollarSign  },
              { label: 'Avg Project',      value: '$2,000',     sub: 'One-time builds',     color: '#5A7A9E',  Icon: TrendingUp  },
              { label: 'Best Retainer',    value: '$2,000/mo',  sub: 'Full-service package', color: '#2A8C68', Icon: BookOpen    },
              { label: 'Top Niche',        value: 'Law Firms',  sub: '$8,000+ potential',   color: '#9E5A9E',  Icon: Users       },
          ...
          <div>
            <div className="flex items-center gap-2.5 mb-3">
              <span className="w-1.5 h-1.5 rounded-full bg-[#9E7A30]" />
              <p className="text-[10px] uppercase tracking-[0.18em] font-semibold text-[#9E7A30]">One-Time Projects</p>
            </div>
            <div className="bg-[#0D0D0D] border border-[#1A1A1A] rounded-2xl overflow-hidden">
              {ONE_TIME.map(({ service, price, note }, i) => (
                ...
                >
                  ...
                  <span className="text-[#9E7A30] text-sm font-semibold tabular-nums whitespace-nowrap flex-shrink-0">{price}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center gap-2.5 mb-3">
              <span className="w-1.5 h-1.5 rounded-full bg-[#2A8C68]" />
              <p className="text-[10px] uppercase tracking-[0.18em] font-semibold text-[#2A8C68]">Monthly Retainers</p>
            </div>
            <div className="bg-[#0D0D0D] border border-[#1A1A1A] rounded-2xl overflow-hidden">
              {RETAINERS.map(({ service, price, note }, i) => (
                ...
                >
                  ...
                  <span className="text-[#2A8C68] text-sm font-semibold tabular-nums whitespace-nowrap flex-shrink-0">{price}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center gap-2.5 mb-3">
              <span className="w-1.5 h-1.5 rounded-full bg-[#5A7A9E]" />
              <p className="text-[10px] uppercase tracking-[0.18em] font-semibold text-[#5A7A9E]">What Each Niche Typically Pays</p>
            </div>
